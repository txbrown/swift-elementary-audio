...
class PanView: NSView {
    ...
    // MARK: - Hit Testing
    // Pass left-click events through to SwiftUI for node/wire dragging.
    // Trackpad scroll/zoom still works via scrollWheel (not affected by hitTest).
    // Right-click and option+drag pan still work when those modifiers are active.
    override func hitTest(_ point: NSPoint) -> NSView? {
        // Right mouse button or option key - handle in this view for panning
        if NSEvent.pressedMouseButtons & (1 << 1) != 0 || NSEvent.modifierFlags.contains(.option) {
            return super.hitTest(point)
        }
        // Left-click: pass through to SwiftUI layer for node/wire interaction
        return nil
    }

    // MARK: - Cursor
    // Use arrow cursor instead of default I-beam text cursor
